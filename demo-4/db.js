...
function initDb(dbPath) {
  ...
  // Migration: add is_favorite column to existing tables that lack it.
  // ALTER TABLE will throw if the column already exists — that's expected.
  try {
...
/**
 * Toggle the is_favorite flag on a link (0 → 1, 1 → 0).
 * Returns the updated row, or null if the id does not exist.
 */
function toggleFavorite(id) {
  const info = db.prepare(
    `UPDATE links SET is_favorite = CASE WHEN is_favorite = 1 THEN 0 ELSE 1 END,
...
/**
 * Return all favorited links, newest first.
 */
function getFavoriteLinks() {
  return db.prepare('SELECT * FROM links WHERE is_favorite = 1 ORDER BY created_at DESC').all();
}
