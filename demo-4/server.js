...
const db = require('./db');
const { validateLink, validatePartialLink, validateId, validateSearchQuery } = require('./validation');
...
// GET /api/links/favorites — list favorited links
app.get('/api/links/favorites', (_req, res) => {
  const links = db.getFavoriteLinks();
  res.json({ links });
});

// GET /api/links/search?q= — search links
app.get('/api/links/search', (req, res) => {
  const check = validateSearchQuery(req.query.q);
  if (!check.valid) {
    return res.status(400).json({ errors: check.errors });
  }

  const links = db.searchLinks(check.query);
  res.json({ links });
});
...
// PUT /api/links/:id/favorite — toggle favorite status
app.put('/api/links/:id/favorite', (req, res) => {
  const check = validateId(req.params.id);
  if (!check.valid) {
    return res.status(400).json({ errors: check.errors });
  }

  const link = db.toggleFavorite(check.id);
  if (!link) {
    return res.status(404).json({ errors: ['Link not found'] });
  }
...
// GET /search — serve the search UI page
